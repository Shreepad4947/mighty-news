...
class _UserDashboardScreenState extends State<UserDashboardScreen> with AfterLayoutMixin<UserDashboardScreen> {
  ...
  Future<void> init() async {
    ...
    if (getIntAsync(THEME_MODE_INDEX) == theme.ThemeModeSystem) {
  ...
  void afterFirstLayout(BuildContext context) {
    ...
    appStore.setLanguage(getStringAsync(theme.LANGUAGE, defaultValue: theme.DefaultLanguage), context: context);
  }
          ...
          bottomNavigationBar: Observer(
            builder: (_) => BottomNavigationBar(
              ...
              items: [
                ...
                BottomNavigationBarItem(
                  icon: appStore.isLoggedIn && getStringAsync(theme.PROFILE_IMAGE).isNotEmpty
                      ? cachedImage(getStringAsync(theme.PROFILE_IMAGE), height: 24, width: 24, fit: BoxFit.cover).cornerRadiusWithClipRRect(15)
                      : Icon(MaterialIcons.person_outline, color: context.theme.iconTheme.color),
                  label: 'Profile',
                  activeIcon: appStore.isLoggedIn && getStringAsync(theme.PROFILE_IMAGE).isNotEmpty
                      ? Container(
                          decoration: BoxDecoration(border: Border.all(color: colorPrimary), shape: BoxShape.circle),
                          child: cachedImage(getStringAsync(theme.PROFILE_IMAGE), height: 24, width: 24, fit: BoxFit.cover).cornerRadiusWithClipRRect(15))
